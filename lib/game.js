...
const gameSlot = async (Xlesy, m, db) => {
	...
	try {
		if (slot1 === slot2 && slot2 === slot3) {
			...
			Xlesy.sendMessage(m.chat, { text: sloth }, { quoted: ftorli })
		...
		console.log(e)
		m.reply('Error!', { quoted: fkontak1 })
	}
}

const gameCasinoSolo = async (Xlesy, m, prefix, db) => {
	try {
		let buatall = 1
		const botNumber = await Xlesy.decodeJid(Xlesy.user.id)
		...
		if (m.args.length < 1) return m.reply(prefix + 'casino <jumlah>\n' + prefix + 'casino 1000', { quoted: fkontak1 })
		if (isNaN(m.args[0])) return m.reply(`Masukkan jumlahnya!\nContoh : ${prefix + m.command} 1000`, { quoted: fkontak1 })
		if (db.users[m.sender].uang >= count * 1) {
			db.users[m.sender].uang -= count * 1
			db.set[botNumber].uang += count * 1
			if (Aku > Kamu) {
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You LOSE*\nKamu kehilangan ${count} Uang`.trim(), { quoted: fkontak1 })
			} else if (Aku < Kamu) {
				db.users[m.sender].uang += count * 2
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nKamu mendapatkan ${count * 2} Uang`.trim(), { quoted: fkontak1 })
			} else {
				db.users[m.sender].uang += count * 1
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*SERI*\nKamu mendapatkan ${count * 1} Uang`.trim(), { quoted: fkontak1 })
			}
		} else {
			m.reply(`Uang kamu tidak mencukupi untuk Casino silahkan *kumpulkan* terlebih dahulu!`, { quoted: fkontak1 })
		}
	} catch (e) {
		console.log(e)
		m.reply('Error!', { quoted: fkontak1 })
...
const gameMerampok = async (m, db) => {
	...
	if (new Date - db.users[m.sender].lastrampok > 3600000) {
		...
		if (!who) return m.reply('Tag salah satu', { quoted: fkontak1 })
		if (!db.users[who]) return m.reply('Target tidak terdaftar di database!', { quoted: fkontak1 })
		if (10000 > db.users[who].uang) return m.reply('Targetnya Kismin ngab🦖', { quoted: fkontak1 })
		db.users[who].uang -= dapat
		db.users[m.sender].uang += dapat
		db.users[m.sender].lastrampok = new Date * 1
		m.reply(`Berhasil Merampok Money Target Sebesar ${dapat}`, { quoted: fkontak1 })
	} else {
		m.reply(`Anda Sudah merampok dan berhasil sembunyi, tunggu ${timers} untuk merampok lagi`, { quoted: fkontak1 })
	}
}

const gameBegal = async (Xlesy, m, db) => {
	...
	if (new Date - user.lastbegal > 3600000) {
		let { key } = await m.reply('Sedang Mencari Pemain...', { quoted: fkontak1 })
		await sleep(2000)
		if (teksnya.no === 0) {
			await m.reply({ text: teksnya.teks, edit: key })
			await m.reply('Gagal Mencari Pemain, Silahkan Coba lagi', { quoted: fkontak1 })
		} else if (teksnya.no === 1) {
			await m.reply({ text: teksnya.teks, edit: key })
			await m.reply(`Kamu Di Bunuh Oleh Pemain\nUang Kamu Di Rampas Sebesar *${randomUang}*`, { quoted: fkontak1 })
			db.users[m.sender].uang -= randomUang
			db.set[botNumber].uang += randomUang * 1
		} else {
			await m.reply({ text: teksnya.teks, edit: key })
			await m.reply(`Berhasil Mendapatkan Uang Sebesar : *${randomUang}*`, { quoted: fkontak1 })
			db.users[m.sender].uang += randomUang
			db.users[m.sender].lastbegal = new Date * 1
		}
	} else {
		m.reply(`Silahkan tunggu *⏱️${timers}* lagi untuk bisa bermain lagi`, { quoted: fkontak1 })
	}
}
...
const transfer = async (m, args, db) => {
	if (args[0] == 'limit') {
		...
		if (!who) return m.reply('Siapa yg mau di transfer?', { quoted: fkontak1 })
		if (db.users[who]) {
			if (db.users[m.sender].limit >= count * 1) {
				try {
					db.users[m.sender].limit -= count * 1
					db.users[who].limit += count * 1
					m.reply(`Berhasil mentransfer limit sebesar ${count}, kepada @${who.split('@')[0]}`, { quoted: fkontak1 })
				} catch (e) {
					db.users[m.sender].limit += count * 1
					m.reply('Gagal Transfer', { quoted: fkontak1 })
				}
			} else {
				m.reply(`Limit tidak mencukupi!!\nLimit mu tersisa : *${db.users[m.sender].limit}*`, { quoted: fkontak1 })
			}
		} else m.reply(`Nomer ${who.split('@')[0]} Bukan User bot!`, { quoted: fkontak1 })
	} else if (args[0] == 'uang') {
		if (!args[1].length > 7) return m.reply(`Transfer Menu :\nExample : ${m.prefix + m.command} limit @tag 11\n• ${m.prefix + m.command} limit @tag jumlah\n• ${m.prefix + m.command} uang @tag jumlah`, { quoted: fkontak1 })
		let count = parseInt(args[2] && args[2].length > 0 ? Math.min(9999999, Math.max(parseInt(args[2]), 1)) : Math.min(1))
		let who = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : args[1] ? (args[1].replace(/[^0-9]/g, '') + '@s.whatsapp.net') : false
		...
		if (db.users[who]) {
			if (db.users[m.sender].uang >= count * 1) {
				try {
					...
					m.reply(`Berhasil mentransfer uang sebesar ${count}, kepada @${who.split('@')[0]}`, { quoted: fkontak1 })
				} catch (e) {
					db.users[m.sender].uang += count * 1
					m.reply('Gagal Transfer', { quoted: fkontak1 })
				}
			} else {
				m.reply(`Uang tidak mencukupi!!\Uang mu tersisa : *${db.users[m.sender].uang}*`, { quoted: fkontak1 })
			}
		} else m.reply(`Nomer ${who.split('@')[0]} Bukan User bot!`, { quoted: fkontak1 })
	} else {
